...
(* The tests *)
let test_string expected ast = Alcotest.(check string) expected expected ast

let test_equal_ty expected ast =
  let check_ty = Alcotest.testable Types.pp_ty Types.equal_ty in
  Alcotest.(check check_ty)
    Types.(Format.asprintf "%a" pp_ty expected)
    expected ast

let test_equal_term expected ast =
  let check_term = Alcotest.testable Terms.pp_term Terms.equal_term in
  Alcotest.(check check_term)
    Terms.(Format.asprintf "%a" pp_term expected)
    expected ast
...
let test_ppshow_opaque () =
  test_string "(Types.PolymorphicType (\"toto\", (Types.TyBoundVar 0)))"
    Types.(Format.asprintf "%a" pp_ty (poly_ty "toto" (fun x -> x)))

let test_ppeq_polyty1 () =
  test_equal_ty (poly_ty "titi" (fun x -> x)) (poly_ty "toto" (fun x -> x))
...
let poly_id = ty_fn "X" (fun _X -> fn "x" _X (fun x -> x))
let test_fn_id () = test_equal_term id (Fun ("x", TyFreeVar "X", Var "x"))

let test_fn_poly_id () =
  test_equal_term poly_id
    (TypeAbstraction ("X", Fun ("x", TyFreeVar "X", Var "x")))
...
let test_fn2 () =
  test_equal_term
    ...
    (Fun
       ( "x",
         TyFreeVar "X",
         Fun ("y", TyFreeVar "X", FunApply (Var "y", Var "x")) ))

let test_letin () =
  test_equal_term
    (letin "x" (Var "a") (fun y -> id $ y))
    (Let ("x", Var "a", FunApply (Fun ("x", TyFreeVar "X", Var "x"), Var "x")))
...
let test_abstract_fn () =
  test_equal_ty
    (Types.abstract "a" (TyFreeVar "a" => TyFreeVar "b"))
    (PolymorphicType ("a", TyBoundVar 0 => TyFreeVar "b"))
...
let test_abstract_poly1 () =
  test_equal_ty
    (Types.abstract "a"
       (TyFreeVar "a"
       => PolymorphicType
            ("b", tuple [ TyFreeVar "c"; TyBoundVar 0; TyFreeVar "a" ])))
    (PolymorphicType
       ( "a",
         TyBoundVar 0
         => PolymorphicType
              ("b", tuple [ TyFreeVar "c"; TyBoundVar 0; TyBoundVar 1 ]) ))
    ...
    (Types.abstract "a"
       ...
       => PolymorphicType
            ( "b",
              tuple
                [
                  PolymorphicType ("a", TyBoundVar 0 => TyBoundVar 0);
                  TyBoundVar 0;
                  TyFreeVar "a";
                ] )))
    (PolymorphicType
  ...
  test_equal_ty
    (Types.abstract "d"
       (poly_ty "b" (fun x ->
            x
            => poly_ty "c" (fun z ->
                   tuple [ poly_ty "a" (fun a -> a => x => z); x; z ]))))
    (PolymorphicType
       ( "d",
         PolymorphicType
           ( "b",
             ...
             => PolymorphicType
                  ( "z",
                    tuple
                      [
                        PolymorphicType
                          ("a", TyBoundVar 0 => TyBoundVar 2 => TyBoundVar 1);
                        TyBoundVar 1;
                        TyBoundVar 0;
                      ] ) ) ))
...
let test_fill_fn () =
  test_equal_ty
    (Types.fill (poly_ty "X" (fun x -> x => TyFreeVar "b")) (TyFreeVar "T"))
    (TyFreeVar "T" => TyFreeVar "b")

let test_fill_poly1 () =
  test_equal_ty
    (Types.fill
       (poly_ty "X" (fun x ->
            TyFreeVar "c" => poly_ty "Y" (fun y -> tuple [ x; y ])))
       (TyFreeVar "T"))
    (TyFreeVar "c" => poly_ty "Y" (fun y -> tuple [ TyFreeVar "T"; y ]))
  ...
  test_equal_ty
    (Types.fill
       (poly_ty "a" (fun x ->
            x
            => poly_ty "b" (fun y ->
                   tuple [ poly_ty "a" (fun z -> z => x); y; x ])))
       (TyFreeVar "T"))
    (TyFreeVar "T"
    => poly_ty "b" (fun y ->
           tuple [ poly_ty "a" (fun z -> z => z); y; TyFreeVar "T" ]))
...
let test_fill_poly3 () =
  test_equal_ty
    ...
    => poly_ty "c" (fun z ->
           tuple
             [
               poly_ty "a" (fun a -> a => TyFreeVar "T" => z); TyFreeVar "T"; z;
             ]))
...
let test_print_ty_fun_very_long () =
  ...
  let t =
    ...
    => (fv "long_variable_name2"
       => (fv "long_variable_name1"
          => (fv "long_variable_name2"
             => (fv "long_variable_name1"
                => (fv "long_variable_name2"
                   => (fv "long_variable_name1"
                      => (fv "long_variable_name 2" => fv "long_variable_name3")
                      ))))))
  in
  test_pretty_print
    "long_variable_name1 ->\n\
...
let test_print_poly_type_complex () =
  let t =
    poly_ty "x1" (fun x ->
        poly_ty "x2" (fun y -> poly_ty "x3" (fun z -> tuple [ z; y; x ])))
  in
...
let test_print_variable () =
  test_pretty_print "toto" (Terms.to_string (Var "toto"))
...
let test_print_fun2 () =
  ...
  test_pretty_print "fun (y: ty1) = z[ty2]" (Terms.to_string t)

let test_print_fun3 () =
  let t =
    fn "x"
      (poly_ty "poly_ty" (fun _x -> fv "ty1" => fv "ty2"))
      (fun _ -> Var "z")
  in
...
let test_print_fun4 () =
  let t =
    fn "x"
      (poly_ty "poly_ty" (fun _x ->
           fv "ty1" => (fv "ty2" => (fv "ty3" => fv "ty4"))))
      (fun _ ->
        fn "y"
          (fv "ty4" => fv "ty5")
          (fun _ -> fn "z" (fv "ty6" => fv "ty7") (fun _ -> Var "z")))
  in
  test_pretty_print
    "fun (x: forall poly_ty. (ty1 -> (ty2 -> (ty3 -> ty4)))) =\n\
    \  (fun (y: ty4 -> ty5) = (fun (z: ty6 -> ty7) = z))" (Terms.to_string t)
...
let test_print_fun_apply2 () =
  let t = Var "x" $ Var "y" $ fn "z" (fv "ty") (fun z -> z $ Var "x") in
  test_pretty_print "(x y) (fun (z: ty) = (z x))" (Terms.to_string t)
...
let test_print_let3 () =
  let t =
    letin "x" (Var "f" $ Var "y") (fun _ ->
        Var "g"
        $ letin "x" (Var "f2" $ Var "y2") (fun _ ->
              letin "x2" (Var "f3" $ Var "y3") (fun _ -> Var "g5" $ Var "toto")))
  in
  test_pretty_print
...
let test_print_type_abstraction2 () =
  ...
  test_pretty_print "fun [tyvar] = (fun (x: ty2) = z)" (Terms.to_string t)
...
let test_print_type_apply2 () =
  let t =
    fn "x" (fv "ty") (fun _ ->
        Var "g" $ letin "x" (Var "f2" $ Var "y2") (fun _ -> Var "y"))
    $! poly_ty "x" (fun _ ->
           tuple [ fv "a"; fv "tyvar3" => fv "tyvar4" ] => fv "b")
  in
  test_pretty_print
    "(fun (x: ty) = (g let x = (f2 y2) in y))[forall x.\n\
    \  ((a * (tyvar3 -> tyvar4)) -> b)]" (Terms.to_string t)

(*****************************************************************************)
(* free_vars tests *)
let checkVarSet = Alcotest.(slist string String.compare)
let set_to_list s = List.of_seq (Terms.VarSet.to_seq s)

let list_to_string l =
  (* prints a list of string as [x1; x2; ...; xn;]*)
  let s =
    List.fold_left (fun x acc -> String.concat "" [ acc; "; "; x ]) "" l
  in
  String.concat "" [ "["; s; "]" ]

let test_free_vars l ast = Alcotest.(check checkVarSet) (list_to_string l) l ast

let test_free_vars_var () =
  test_free_vars [ "a" ] (set_to_list (Terms.free_vars (Var "a")))

let test_free_vars_fun1 () =
  let t = fn "b" (fv "ty") (fun _ -> Var "a") in
  test_free_vars [ "a" ] (set_to_list (Terms.free_vars t))

let test_free_vars_fun2 () =
  let t = fn "b" (fv "ty") (fun x -> fn "a" (fv "ty") (fun _ -> x)) in
  test_free_vars [] (set_to_list (Terms.free_vars t))

let test_free_vars_funApply1 () =
  let t =
    fn "a" (fv "ty") (fun _ -> Var "b") $ fn "b" (fv "ty") (fun _ -> Var "a")
  in
  test_free_vars [ "a"; "b" ] (set_to_list (Terms.free_vars t))
...
let test_free_vars_funApply2 () =
  let t =
    ...
    $ (fn "z" (fv "ty") (fun _ -> Var "x") $ fn "u" (fv "ty") (fun x -> x))
  in
  test_free_vars [ "z"; "x"; "y"; "v" ] (set_to_list (Terms.free_vars t))

let test_free_vars_Let1 () =
  let t =
    letin "a"
      (fn "b" (fv "ty") (fun _ -> Var "a") $ letin "c" (Var "d") (fun x -> x))
      (fun _ -> Var "e")
  in
  test_free_vars [ "a"; "d"; "e" ] (set_to_list (Terms.free_vars t))

let test_free_vars_Let2 () =
  let t =
    letin "x" (Var "a") (fun x ->
        letin "y" (fn "z" (fv "ty") (fun _ -> Var "b" $ x) $ x) (fun y -> y $ x))
  in
  test_free_vars [ "a"; "b" ] (set_to_list (Terms.free_vars t))
...
let test_free_var_type_abstraction () =
  let t =
    ty_fn "ty" (fun x ->
        letin "a"
          (fn "b" x (fun _ -> Var "e") $ letin "c" (Var "d") (fun y -> y))
          (fun z -> z))
  in
  test_free_vars [ "d"; "e" ] (set_to_list (Terms.free_vars t))

let test_free_var_type_apply () =
  let t =
    letin "a"
      (fn "b" (fv "ty") (fun _ -> Var "a") $ letin "c" (Var "d") (fun c -> c))
      (fun _ -> Var "e")
    $! fv "ty"
  in
  test_free_vars [ "a"; "d"; "e" ] (set_to_list (Terms.free_vars t))
