...
let test_pp_term () =
  ...
  test_string
    (Format.asprintf "(Terms.Var %a)" pp toto)
    (Format.asprintf "%a" pp_term (Var toto))
...
let test_fn_id () = test_equal_term id (Fun (id_x, TyFreeVar id_t, Var id_x))

let test_fn_poly_id () =
  test_equal_term
    (TypeAbstraction
       (poly_id_t, Fun (poly_id_x, TyFreeVar poly_id_t, Var poly_id_x)))
    poly_id

let test_fn2 () =
  test_equal_term
    (Fun (x, TyFreeVar tt, Fun (y, TyFreeVar tt, FunApply (Var y, Var x))))
...
let test_fn3 () =
  ...
  test_equal_term
    (Fun
       ( x2,
         TyFreeVar tt,
         FunApply (Var x2, Fun (x1, TyFreeVar tt, FunApply (Var x2, Var x1))) ))
    (fn x2 (TyFreeVar tt) (fun x -> x $ fn x1 (TyFreeVar tt) (fun z -> x $ z)))

let test_letin () =
  test_equal_term
    (Let (x, Var a, FunApply (id, Var x)))
...
let test_print_variable () =
  let toto = fresh "toto" in
  test_pretty_print (pa toto) (Var toto)

let test_print_fun1 () =
  let t = fn x (fv tt) (fun _ -> Var a) in
  test_pretty_print (Format.sprintf "fun (%s: %s) = %s" (pa x) (pa tt) (pa a)) t

let test_print_fun2 () =
  let t = fn x (fv tt) (fun _ -> Var a $! fv ts) in
  test_pretty_print
    (Format.asprintf "fun (%s: %s) = %s[%s]" (pa x) (pa tt) (pa a) (pa ts))
    t

let test_print_fun3 () =
  let t = fn x (poly_ty tx (fun _x -> fv ts => fv tt)) (fun _ -> Var a) in
  test_pretty_print
...
let test_print_fun4 () =
  ...
  let t =
    fn x
      ...
      (fun _ ->
        fn y (fv t4 => fv t5) (fun _ -> fn z (fv t6 => fv t7) (fun _ -> Var z)))
  in
  test_pretty_print
    (Format.sprintf
       "fun (%s:\n\
...
let test_print_fun_apply2 () =
  let t = Var a $ Var b $ fn x (fv tt) (fun x -> x $ Var a) in
  test_pretty_print
...
let test_print_let1 () =
  let t = letin x (Var a) (fun _ -> Var b) in
  test_pretty_print (Format.sprintf "let %s = %s in %s" (pa x) (pa a) (pa b)) t

let test_print_let2 () =
  let t = letin x (Var f $ Var a) (fun _ -> Var g $ Var b) in
  test_pretty_print
...
let test_print_let3 () =
  ...
  let toto = fresh "toto" in
  let t =
    letin x (Var f $ Var a) (fun _ ->
        Var g
        $ letin x (Var f2 $ Var b) (fun _ ->
              letin y (Var f3 $ Var c) (fun _ -> Var f4 $ Var toto)))
  in
  test_pretty_print
    (Format.sprintf
       "let %s = (%s %s) in\n\
...
let test_print_type_apply2 () =
  ...
  let t4 = fresh "T4" in
  let t =
    fn x (fv tt) (fun _ -> Var g $ letin x (Var h $ Var b) (fun _ -> Var a))
...
let test_free_vars_var () =
  let a = fresh "a" in
  test_free_vars [ a ] (Var a)

let test_free_vars_fun1 () =
  let a = fresh "a" in
  let t = fn (fresh "b") (fv (fresh "ty")) (fun _ -> Var a) in
  test_free_vars [ a ] t
...
let test_free_vars_funApply1 () =
  ...
  let a = fresh "a" in
  let b = fresh "b" in
  let t = fn a (fv ty) (fun _ -> Var b) $ fn b (fv ty) (fun _ -> Var a) in
...
let test_free_vars_funApply2 () =
  ...
  let t =
    ...
    $ (fn z (fv ty) (fun _ -> Var x) $ fn (fresh "u") (fv ty) (fun x -> x))
  in
...
let test_free_vars_Let1 () =
  ...
  let t =
    letin (fresh "a")
      (fn (fresh "b") (fv (fresh "ty")) (fun _ -> Var a)
      $ letin (fresh "c") (Var d) (fun x -> x))
      (fun _ -> Var e)
  in
...
let test_free_vars_Let2 () =
  ...
  let b = fresh "b" in
  let t =
    letin (fresh "x") (Var a) (fun x ->
        letin (fresh "y")
          (fn (fresh "z") (fv (fresh "ty")) (fun _ -> Var b $ x) $ x)
          (fun y -> y $ x))
  in
  test_free_vars [ a; b ] t
...
let test_free_var_type_abstraction () =
  ...
  let t =
    ty_fn (fresh "ty") (fun x ->
        letin (fresh "a")
          (fn (fresh "b") x (fun _ -> Var e)
          $ letin (fresh "c") (Var d) (fun y -> y))
          (fun z -> z))
  in
...
let test_free_var_type_apply () =
  ...
  let t =
    letin (fresh "a")
      (fn (fresh "b") (fv ty) (fun _ -> Var a)
      $ letin (fresh "c") (Var d) (fun c -> c))
      (fun _ -> Var e)
    $! fv ty
  in
  test_free_vars [ a; d; e ] t
