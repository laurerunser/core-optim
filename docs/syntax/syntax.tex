...
\begin{proof}
  ...
  \begin{proof}
    ...
    \begin{proof}
      \step{label-2.1.1}{$\go {\scoped {(\tfun \aVar {\aType[2]} {\aTerm[3]})} \env} \aStack = \go {\scoped {\aTerm[3]} {\envextend \env \aVar {\subs \aBase {\env'}}}} {\aStack'}$}
      \begin{proof}
        \pf\ By definition of \textsf{go}
      \end{proof}
      \step{label-2.1.2}{$\simplBeta[*] {\stackapp {\aStack'} {\subs {\aTerm[3]} {\envextend \env \aVar {\subs \aBase {\env'}}}}} {\betaNormal {\go {\scoped {\aTerm[3]} {\envextend \env \aVar {\subs \aBase {\env'}}}} {\aStack'}}}$ and $\judge \aContext {\go {\scoped {\aTerm[3]} {\envextend \env \aVar {\subs \aBase {\env'}}}} {\aStack'}} {\aType[1]}$}
      \begin{proof}
        \step{label-2.1.2.1}{$\exists \aType[3]. \judge \aContext {\subs {\aTerm[3]} {\envextend \env \aVar {\subs \aBase {\env'}}}} {\aType[3]}$ and $\frameJudge \aContext {\aStack'} {\aType[3]} {\aType[1]}$}
        \begin{proof}
        ...
        \end{proof}
      \end{proof}
      \step{label-2.1.3}{$\simplBeta {\stackapp {(\nonEmptyStack {\aFunFrame[\env']} {\aStack'})} {\subs {(\tfun \aVar {\aType[2]} {\aTerm[3]})} \env}}{\stackapp {\aStack'} {\subs {\aTerm[3]} {\envextend \env \aVar {\subs \aBase {\env'}}}}}$}
      \begin{proof}
        \pf\ \\
        $\begin{aligned}
  ...
  \begin{proof}
    ...
    \begin{proof}
      \pf\ By definition of \textsf{go}
    \end{proof}
    \step{label-4.2}{$\simplBeta[*] {\stackapp {(\nonEmptyStack {\aFunFrame[\env]} \aStack)} {\subs {\aTerm[3]} \env}} \betaNormal{{\go {\scoped {\aTerm[3]} \env} {(\nonEmptyStack {\aFunFrame[\env]} \aStack)}}}$ and $ \judge \aContext {\go {\scoped {\aTerm[3]} \env} {(\nonEmptyStack {\aFunFrame[\env]} \aStack)}} {\aType[1]}$}
    \begin{proof}
      \step{label-4.2.1}{$\exists \aType[2]. \judge \aContext {\subs {\aTerm[3]} \env} {\aType[2] \to \aType}$}
      \begin{proof}
        \pf\ By substitution and inversion of \ref{pf:term-typed}
      \end{proof}
      \step{label-4.2.2}{$(\nonEmptyStack {\aFunFrame[\env]} \aStack)$ is fully simplifiable}
      \begin{proof}
        \pf\ By \ref{pf:stack-simpl}
      \end{proof}
      \step{label-4.2.3}{$\frameJudge \aContext {(\nonEmptyStack {\aFunFrame[\env]} \aStack)} {\aType[2] \to \aType} {\aType[1]}$}
      \begin{proof}
        \pf\ By inversion of \ref{pf:stack-typed}
      \end{proof}
      \qedstep
  ...
  \begin{proof}
    ...
    \step{label-6.4}{$\simplBeta[*]{\stackapp {(\nonEmptyStack {\aIteFrameAll {\aTerm[4]} {\aTerm[5]} \env} \aStack)} {\subs {\aTerm[3]} \env}} {\betaNormal{\go {\scoped {\aTerm[3]} \env {(\nonEmptyStack {\aIteFrameAll {\aTerm[4]} {\aTerm[5]} \env} \aStack)}}}$ and $\judge \aContext {\go {\scoped {\aTerm[3]} \env {(\nonEmptyStack {\aIteFrameAll {\aTerm[4]} {\aTerm[5]} \env} \aStack)}}} {\aType[1]}$}}
    \begin{proof}
      \step{label-6.4.1}{$\judge \aContext {\subs {\aTerm[3]} \env} \tyBool$}
      \begin{proof}
        \pf\ By substitution and inversion of \ref{pf:term-typed}
      \end{proof}
      \step{label-6.4.2}{$(\nonEmptyStack {\aIteFrameAll {\aTerm[4]} {\aTerm[5]} \env} \aStack)$ is fully simplifiable}
      \begin{proof}
        \pf\ TODO
      \end{proof}
      \step{label-6.4.3}{$\frameJudge \aContext {(\nonEmptyStack {\aIteFrameAll {\aTerm[4]} {\aTerm[5]} \env} \aStack)} \tyBool {\aType[1]}$}
      \begin{proof}
