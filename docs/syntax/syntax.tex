...
\begin{proof}
  \assume{\begin{pfenum}
      ...
      \item $\subs \aTerm \env$ is well-typed in the context $\aContext$ \label{pf:term-typed}
      \item $\aStack$ is well-typed in the context $\aContext$ for the type of $\subs \aTerm \env$ \label{pf:stack-typed}
    \end{pfenum}}
  \prove{$\simplBeta[*] {\stackapp \aStack {\subs \aTerm \env}} {\betaNormal {\go {\scoped \aTerm \env} \aStack}}$ and $\go {\scoped \aTerm \env} \aStack$ has the type of $\stackapp \aStack {\subs \aTerm \env}$ in the context $\Gamma$}
