...
Similarily, $\scoped \aType \env$ denotes the pair of the type $\aType$ and its environment $\env$. \\
The domain of an environement is written $D(\env)$. \\
$D(\env) = \{\aVar. \exists \aBase. \envextend \rho \aVar \aBase\} \cup \{\aTypeVar. \exists \aType. \envextend \rho \aTypeVar \aType\}$


\subsection{Well scoped} \label{well-scoped-term}
$\scoped \aType \env$ is well-scoped if $FV_{typeVar}(\aType) \subseteq D(\env)$.

$\scoped \aTerm \env$ is well-scoped if:
\begin{enumerate}
  \item $FV(\aTerm) \subseteq D(\env)$
  \item $FV_{typeVar}(\aTerm) \subseteq D(\env)$
\end{enumerate}
...
$
\subsection{Stack application} \label{stack-evaluation}
\begin{align*}
...
\aRule  {\judgecheck \aContext {\aTerm[1]} {\aType[2]} \\
...
\begin{proof}
  ...
  \begin{proof}
    \step{label-1.1}{\case{$\subs \aBase \env = \true$ and $\aStack = \nonEmptyStack \aIteFrameScoped {\aStack'}$}}
    \begin{proof}
      \step{label-1.1.1}{$\go {\scoped \aBase \env} {(\nonEmptyStack \aIteFrameScoped {\aStack'})} = \go {\scoped {\aTerm[1]} {\env[1]} {\aStack'}}$}
      \begin{proof}
        \pf\ By definition of \textsf{go}
      \end{proof}
      \step{label-1.1.2}{$\simplBeta[*] {\stackapp {\aStack'} {\subs {\aTerm[1]} {\env[1]}}} {\betaNormal {\go {\scoped {\aTerm[1]} {\env[1]} {\aStack'}}}}$ 
                        and $\judge \aContext {\go {\scoped {\aTerm[1]} {\env[1]} {\aStack'}}} {\aType[1]}$}
      \begin{proof}
        \step{label-1.1.2.1}{$\exists \aType[2].$ 
          \begin{enumerate}
            \item $\judge \aContext {\subs {\aTerm[1]} {\env[1]}} {\aType[2]}$ 
            \item and $\frameJudge \aContext {\aStack'} {\aType[2]} {\aType[1]}$
          \end{enumerate}
            }
        \begin{proof}
        ...
        \end{proof}
      \end{proof}
      \step{label-1.1.3}{$\simplBeta {\stackapp {(\nonEmptyStack \aIteFrameScoped {\aStack'})} \true} {\stackapp {\aStack'} {\subs {\aTerm[1]} {\env[1]}}}$}
      \begin{proof}
        \pf\ By stack application \ref{stack-evaluation} and definition of plug.
      \end{proof}
      \qedstep
      \begin{proof}
        \pf\
        \begin{enumerate}
          \item $\judge \aContext {\go {\scoped \aBase \env} {(\nonEmptyStack \aIteFrameScoped {\aStack'})}} {\aType[1]}$ by \stepref{label-1.1.1} and \stepref{label-1.1.2}
          \item We get the following equations:
            \begin{align*}
            \subs {\aStack} {\scoped \aBase {\env}} = \stackapp {(\nonEmptyStack \aIteFrameScoped {\aStack'})} \true && \text{ by case analysis \stepref{label-1.1} and \stepref{label-1.1.2}}
            \\ \rightsquigarrow^{*} {\subs {\aStack'} {\scoped \aTerm[1] {\env[1]}}} && \text{by \stepref{label-1.1.3}}
            \\ \rightsquigarrow^{*} {\go {\scoped {\aTerm[1]} {\env[1]}} {\aStack'}} && \text{by \stepref{label-1.1.2}}
            \\ = {\go {\scoped {\aBase} {\env}} {(\nonEmptyStack \aIteFrameScoped {\aStack'})}} && \text{by \stepref{label-1.1.1}}
            \end{align*}  
        \end{enumerate}
      \end{proof}
    \end{proof}
    ...
    \begin{proof}
      \step{label-1.2.1}{$\go {\scoped \aBase \env} {(\nonEmptyStack \aIteFrameScoped {\aStack'})} = \go {\scoped {\aTerm[2]} {\env[2]} {\aStack'}}$}
      \begin{proof}
        \pf\ By definition of \textsf{go}
      \end{proof}
      \step{label-1.2.2}{$\simplBeta[*] {\stackapp {\aStack'} {\subs {\aTerm[2]} {\env[2]}}} {\betaNormal {\go {\scoped {\aTerm[2]} {\env[2]} {\aStack'}}}}$ 
                        and $\judge \aContext {\go {\scoped {\aTerm[2]} {\env[2]} {\aStack'}}} {\aType[1]}$}
      \begin{proof}
        \step{label-1.2.2.1}{$\exists \aType[2].$ 
          \begin{enumerate}
            \item $\judge \aContext {\subs {\aTerm[2]} {\env[2]}} {\aType[2]}$ 
            \item and $\frameJudge \aContext {\aStack'} {\aType[2]} {\aType[1]}$
          \end{enumerate}
            }
        \begin{proof}
        ...
        \end{proof}
      \end{proof}
      \step{label-1.2.3}{$\simplBeta {\stackapp {(\nonEmptyStack \aIteFrameScoped {\aStack'})} \true} {\stackapp {\aStack'} {\subs {\aTerm[2]} {\env[2]}}}$}
      \begin{proof}
        \pf\ By stack application \ref{stack-evaluation} and definition of plug.
      \end{proof}
      \qedstep
      \begin{proof}
        \pf\
        \begin{enumerate}
          \item $\judge \aContext {\go {\scoped \aBase \env} {(\nonEmptyStack \aIteFrameScoped {\aStack'})}} {\aType[1]}$ by \stepref{label-1.2} and \stepref{label-1.2.2}
          \item We get the following equations:
            \begin{align*}
            \subs {\aStack} {\scoped \aBase {\env}} = \stackapp {(\nonEmptyStack \aIteFrameScoped {\aStack'})} \false && \text{by case analysis \stepref{label-1.2} and \stepref{label-1.2.2}}
            \\ \rightsquigarrow^{*} {\subs {\aStack'} {\scoped \aTerm[2] {\env[2]}}} && \text{by \stepref{label-1.2.3}}
            \\ \rightsquigarrow^{*} {\go {\scoped {\aTerm[2]} {\env[2]}} {\aStack'}} && \text{by \stepref{label-1.2.2}}
            \\ = {\go {\scoped {\aBase} {\env}} {(\nonEmptyStack \aIteFrameScoped {\aStack'})}} && \text{by \stepref{label-1.2.1}}
            \end{align*}  
        \end{enumerate}
      \end{proof}
    \end{proof}
  ...
  \begin{proof}
    ...
    \begin{proof}
      ...
      \begin{proof}
        ...
        \begin{proof}
          ...
          \end{proof}
        \end{proof}
        \step{label-2.1.2.2}{$\aStack'$ is fully-simplified}
        \begin{proof}
          \pf\ By \ref{pf:term-in-stack}
        \end{proof}
        \step{label-2.1.2.3}{$\judge \aContext {\subs {\aTerm[3]} {\envextend \env \aVar \aBase}} {\aType[3]}$ and $\frameJudge \aContext {\aStack'} {\aType[3]} {\aType[1]}$}
        \begin{proof}
          \step{label-2.1.2.3.1}{$\judge {\ctxtExtend \aContext {\aVar[1]} {\subs {\aType[2]} \env}} {\subs {\aTerm[3]} {\envextend \env \aVar {\aVar[1]}}} \aType[3]$ with $\aVar[1] \notin FV(t) \cup FV(\env)$}
          \begin{proof}
          ...
          \end{proof}
          \step{label-2.1.2.3.2}{$\judge \aContext \aBase {\subs {\aType[2]} \env}$ and $\frameJudge \aContext {\aStack'} {\aType[3]} {\aType[1]}$}
          \begin{proof}
    ...
    \begin{proof}
      ...
      \begin{proof}
        \pf\ By definition of \textsf{go}
      \end{proof}
      \step{label-2.2.2}{$\simplBeta[*] {\subs {\aTerm[3]} {\envextend \env \aVar {\aVar[1]}}} {\betaNormal {\go {\scoped {\aTerm[3]} {\envextend \env \aVar {\aVar[1]}}} \emptyStack}}$ and $\judge {\ctxtExtend \aContext {\aVar[1]} {\subs {\aType[2]} \env}} {\go {\scoped {\aTerm[3]} {\envextend \env \aVar {\aVar[1]}}} \emptyStack} {\aType[3]}$}
      \begin{proof}
        ...
        \begin{proof}
          ...
          \end{proof}
        \end{proof}
        \step{label-2.2.2.2}{$\emptyStack$ is fully-simplified}
        \begin{proof}
          \pf\ By the definition of fully-simplified stack
        \end{proof}
        \step{label-2.2.2.3}{$\judge {\ctxtExtend \aContext {\aVar[1]} {\subs {\aType[2]} \env}} {\subs {\aTerm[3]} {\envextend \env \aVar {\aVar[1]}}} {\aType[3]}$}
        \begin{proof}
          \pf\ By substitution, \toplevel:\ref{pf:term-typed} and \toplevel:\ref{pf:stack-typed}
        \end{proof}
        \step{label-2.2.2.4}{$\frameJudge {\ctxtExtend \aContext {\aVar[1]} {\subs {\aType[2]} \env}} \emptyStack {\aType[3]} {\aType[3]}$}
        \begin{proof}
      ...
      \end{proof}
      \step{label-2.2.5}{$\simplBeta[*] {\stackapp \aStack {\subs {(\tfun \aVar {\aType[2]} {\aTerm[3]})} \env}} {\stackapp \aStack {\tfun {\aVar[1]} {\subs {\aType[2]} \env} {\go {\scoped {\aTerm[3]} {\envextend \env \aVar {\aVar[1]}}} \emptyStack}}}$ with $\aVar[1] \notin FV(\aTerm) \cup FV(\env)$}
      \begin{proof}
  ...
  \begin{proof}
    ...
    \begin{proof}
      ...
      \step{label-3.2.5}{$\simplBeta[*] {\stackapp \aStack {\subs {(\ttyfun \aTypeVar {\aTerm[3]})} \env}} {\stackapp \aStack {\ttyfun {\aTypeVar[1]} {\go {\scoped {\aTerm[3]} {\envextend \env \aTypeVar {\aTypeVar[1]}}} \emptyStack}}}$ with $\aVar[1] \notin FV(\aTerm) \cup FV(\env)$}
    \end{proof}
  \end{proof}
  ...
  \begin{proof}
    ...
    \begin{proof}
      ...
      \begin{proof}
        ...
        \end{proof}
      \end{proof}
      \step{label-4.2.2}{$(\nonEmptyStack {\aFunFrame[\subs \aBase \env]} \aStack)$ is fully-simplified}
      \begin{proof}
        ...
        \end{proof}
      \end{proof}
      \step{label-4.2.3}{$\judge \aContext {\subs {\aTerm[3]} \env} {\aType[2] \to \aType}$}
      \begin{proof}
        \pf\ By substitution and \toplevel:\ref{pf:term-typed},
  ...
  \begin{proof}
    ...
    \begin{proof}
      ...
      \begin{proof}
        ...
        \end{proof}
      \end{proof}
      \step{label-6.2.2}{$\emptyStack$ is fully-simplified}
      \begin{proof}
        \pf\ By the definition of fully-simplified stack
      \end{proof}
      \step{label-6.2.3}{$\judge \aContext {\subs {\aTerm[4]} \env} {\aType}$}
      \begin{proof}
        \begin{proof}
          \pf\ By substitution and \toplevel:\ref{pf:term-typed}
        \end{proof}
      \end{proof}
      \step{label-6.2.4}{$\frameJudge \aContext \emptyStack \aType \aType$}
      \begin{proof}
        \pf\ By empty stack rule \ref{stack-typechecker}
      \end{proof}
      \qedstep
    ...
    \step{label-6.4}{$\simplBeta[*]{\stackapp {(\nonEmptyStack {\aIteFrameAll {(\go {\scoped {\aTerm[4]} \env} \emptyStack)} {(\go {\scoped {\aTerm[5]} \env} \emptyStack)}} \aStack)} {\subs {\aTerm[3]} \env}} {\betaNormal{\go {\scoped {\aTerm[3]} \env {(\nonEmptyStack {\aIteFrameAll {(\go {\scoped {\aTerm[4]} \env} \emptyStack)} {(\go {\scoped {\aTerm[5]} \env} \emptyStack)}} \aStack)}}}}$ and $\judge \aContext {\go {\scoped {\aTerm[3]} \env {(\nonEmptyStack {\aIteFrameAll {(\go {\scoped {\aTerm[4]} \env} \emptyStack)} {(\go {\scoped {\aTerm[5]} \env} \emptyStack)}} \aStack)}}} {\aType[1]}$}
    \begin{proof}
      ...
      \begin{proof}
        ...
        \end{proof}
      \end{proof}
      \step{label-6.4.2}{$(\nonEmptyStack {\aIteFrameAll {(\go {\scoped {\aTerm[4]} \env} \emptyStack)} {(\go {\scoped {\aTerm[5]} \env} \emptyStack)}} \aStack)$ is fully-simplified}
      \begin{proof}
        \pf\ By \ref{pf:term-in-stack}, \stepref{label-6.2} and \stepref{label-6.3}
      \end{proof}
      \step{label-6.4.3}{$\judge \aContext {\subs {\aTerm[3]} \env} \tyBool$}
      \begin{proof}
        \pf\ By substitution and \toplevel:\ref{pf:term-typed}
      \end{proof}
      \step{label-6.4.4}{$\frameJudge \aContext {(\nonEmptyStack {\aIteFrameAll {(\go {\scoped {\aTerm[4]} \env} \emptyStack)} {(\go {\scoped {\aTerm[5]} \env} \emptyStack)}} \aStack)} \tyBool {\aType[1]}$}
      \begin{proof}
        \pf\ By \toplevel:\ref{pf:stack-typed}
      \end{proof}
      \qedstep
  ...
  \begin{proof}
    ...
    \step{label-7.6}{$\simplBeta[*] {\stackapp \aStack {\subs {(\tlet \aVar {\aTerm[3]} {\aTerm[4]})} \env}} {\stackapp \aStack {(\tlet {\aVar[1]} {\go {\scoped {\aTerm[3]} \env} \emptyStack} {\go {\scoped {\aTerm[4]} {\envextend \env \aVar {\aVar[1]}}} \emptyStack})}} \text{ with } \aVar[1] \notin FV(\aTerm) \cup FV(\env)$}
  \end{proof}
